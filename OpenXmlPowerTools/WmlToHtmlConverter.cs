...
namespace OpenXmlPowerTools
{
    ...
    public static class WmlToHtmlConverter
    {
            ...
            decimal currentMarginLeft)
        {
            ...
            // Transform altChunk:html to h:div element.
            if (element.Name == W.altChunk && element.Attribute(R.id) != null)
            {
                var id = element.Attribute(R.id).Value;
                var Part = wordDoc.MainDocumentPart.GetPartById(id);
                if (Part != null && Part.ContentType.EndsWith("html"))
                {
